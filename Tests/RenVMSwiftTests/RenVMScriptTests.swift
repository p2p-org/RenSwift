...
class RenVMScriptTests: XCTestCase {
    func testGatewayScript() throws {
        ...
        let gS = Script.gatewayScript(gGubKeyHash: Data(gGubKeyHash), gHash: Data(gHash))
    ...
    func testCreateAddressByteArray() throws {
        ...
        let addressBytes = Script.createAddressByteArray(gGubKeyHash: gGubKeyHash, gHash: Data(gHash), prefix: Data([prefix]))
    ...
    func testChecksum() throws {
        ...
        XCTAssertEqual(Base58.encode(Script.checksum(hash: Data(hash)).bytes), "2vuP8n")
    }
}
