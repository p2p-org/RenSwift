...
class SolanaRenProgramTests: XCTestCase {
    func testCreateInstructionWithEthAddress2() throws {
        ...
        let instruction = SolanaChain.RenProgram.createInstructionWithEthAddress2(ethAddress: Data(ethAddress), message: Data(message), signature: Data(signature), recoveryId: recoveryId)
    ...
    func testBurnInstruction() throws {
        ...
        let instruction = SolanaChain.RenProgram.burnInstruction(
            account: publicKey,
            source: publicKey,
